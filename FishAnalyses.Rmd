---
title: "Attribution Workshop Fish Analyses"
author: "Emma Jones"
output: html_notebook
---
  
This notebook walks users through the process of organizing data and running the CDF, GAM, and TITAN models on VDEQ fish data (and data from other entities) for the VDEQ species attribution workshop.
...
Read in taxa data. Split dataset into environmental and taxa information. This dataset was created by merging all EDAS fish data with probmetrics dataset 2001-2014.
```{r, echo=FALSE}
...
tolerance_bioregion <- function(spdata, envdata,  sp.siteid="Sample.ID", species="GENUS", 
  ...
  for (i in 1:ntaxa) {
    ...
    if(plot.pdf) {
      ## Special Plot for log scale bc I can't get the original log scale to work properly 05212018
        if(emmaLOG==TRUE){
          file_name <- paste('FinalDataAndPlots/Fish/Bioregion/',bioregion,'/',lim,'/',xvar,'/',tnames[i],'.jpg',sep='')
          jpeg(file_name)
          par(mar =mar, oma=c(1.5, 0.5, 2,0.5) )
          typ <- ifelse(samplen >= cutoff, "l", "n")
          if(xvar== "wshdImpPCT"){xrange[1] <- c(0.5)} # can't have 0 for log x scale ######################################
...
# Save CDF data
cdfdataDO <- CDF(merge(fishTaxa,select(envData,UID,DO),by='UID') %>%
                   filter(!is.na(DO)),'DO')
cdfdataDO_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,DO),by='UID') %>%
                           filter(!is.na(DO)),'DO')
cdfdataDO_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,DO),by='UID') %>%
                           filter(!is.na(DO)),'DO')
                                  
cdfdataDO_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,DO),by='UID') %>%
                           filter(!is.na(DO)),'DO')
saveRDS(cdfdataDO,'FinalDataAndPlots/Fish/CDF/cdfdataDO.RDS')
saveRDS(cdfdataDO_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdataDO.RDS')
saveRDS(cdfdataDO_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdataDO.RDS')
saveRDS(cdfdataDO_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdataDO.RDS')

cdfdatapH <- CDF(merge(fishTaxa,select(envData,UID,pH),by='UID') %>%
                   filter(!is.na(pH)),'pH')
cdfdatapH_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,pH),by='UID') %>%
                           filter(!is.na(pH)),'pH')
cdfdatapH_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,pH),by='UID') %>%
                           filter(!is.na(pH)),'pH')
                                  
cdfdatapH_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,pH),by='UID') %>%
                           filter(!is.na(pH)),'pH')
saveRDS(cdfdatapH,'FinalDataAndPlots/Fish/CDF/cdfdatapH.RDS')
saveRDS(cdfdatapH_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdatapH.RDS')
saveRDS(cdfdatapH_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdatapH.RDS')
saveRDS(cdfdatapH_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdatapH.RDS')

cdfdataSpCond <- CDF(merge(fishTaxa,select(envData,UID,SpCond),by='UID') %>%
                   filter(!is.na(SpCond)),'SpCond')
cdfdataSpCond_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,SpCond),by='UID') %>%
                           filter(!is.na(SpCond)),'SpCond')
cdfdataSpCond_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,SpCond),by='UID') %>%
                           filter(!is.na(SpCond)),'SpCond')
                                  
cdfdataSpCond_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,SpCond),by='UID') %>%
                           filter(!is.na(SpCond)),'SpCond')
saveRDS(cdfdataSpCond,'FinalDataAndPlots/Fish/CDF/cdfdataSpCond.RDS')
saveRDS(cdfdataSpCond_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdataSpCond.RDS')
saveRDS(cdfdataSpCond_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdataSpCond.RDS')
saveRDS(cdfdataSpCond_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdataSpCond.RDS')
                         
cdfdataLRBS <- CDF(merge(fishTaxa,select(envData,UID,LRBS),by='UID') %>%
                   filter(!is.na(LRBS)),'LRBS')
cdfdataLRBS_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,LRBS),by='UID') %>%
                           filter(!is.na(LRBS)),'LRBS')
cdfdataLRBS_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,LRBS),by='UID') %>%
                           filter(!is.na(LRBS)),'LRBS')
                                  
cdfdataLRBS_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,LRBS),by='UID') %>%
                           filter(!is.na(LRBS)),'LRBS')
saveRDS(cdfdataLRBS,'FinalDataAndPlots/Fish/CDF/cdfdataLRBS.RDS')
saveRDS(cdfdataLRBS_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdataLRBS.RDS')
saveRDS(cdfdataLRBS_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdataLRBS.RDS')
saveRDS(cdfdataLRBS_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdataLRBS.RDS')


cdfdataTP <- CDF(merge(fishTaxa,select(envData,UID,TP),by='UID') %>%
                   filter(!is.na(TP)),'TP')
cdfdataTP_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,TP),by='UID') %>%
                           filter(!is.na(TP)),'TP')
cdfdataTP_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,TP),by='UID') %>%
                           filter(!is.na(TP)),'TP')
                                  
cdfdataTP_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,TP),by='UID') %>%
                           filter(!is.na(TP)),'TP')
saveRDS(cdfdataTP,'FinalDataAndPlots/Fish/CDF/cdfdataTP.RDS')
saveRDS(cdfdataTP_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdataTP.RDS')
saveRDS(cdfdataTP_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdataTP.RDS')
saveRDS(cdfdataTP_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdataTP.RDS')

cdfdataTN <- CDF(merge(fishTaxa,select(envData,UID,TN),by='UID') %>%
                   filter(!is.na(TN)),'TN')
cdfdataTN_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,TN),by='UID') %>%
                           filter(!is.na(TN)),'TN')
cdfdataTN_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,TN),by='UID') %>%
                           filter(!is.na(TN)),'TN')
                                  
cdfdataTN_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,TN),by='UID') %>%
                           filter(!is.na(TN)),'TN')
saveRDS(cdfdataTN,'FinalDataAndPlots/Fish/CDF/cdfdataTN.RDS')
saveRDS(cdfdataTN_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdataTN.RDS')
saveRDS(cdfdataTN_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdataTN.RDS')
saveRDS(cdfdataTN_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdataTN.RDS')

cdfdataTotHab <- CDF(merge(fishTaxa,select(envData,UID,TotHab),by='UID') %>%
                   filter(!is.na(TotHab)),'TotHab')
cdfdataTotHab_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,TotHab),by='UID') %>%
                           filter(!is.na(TotHab)),'TotHab')
cdfdataTotHab_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,TotHab),by='UID') %>%
                           filter(!is.na(TotHab)),'TotHab')
                                  
cdfdataTotHab_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,TotHab),by='UID') %>%
                           filter(!is.na(TotHab)),'TotHab')
saveRDS(cdfdataTotHab,'FinalDataAndPlots/Fish/CDF/cdfdataTotHab.RDS')
saveRDS(cdfdataTotHab_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdataTotHab.RDS')
saveRDS(cdfdataTotHab_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdataTotHab.RDS')
saveRDS(cdfdataTotHab_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdataTotHab.RDS')

cdfdatawshdImpPCT <- CDF(merge(fishTaxa,select(envData,UID,wshdImpPCT),by='UID') %>%
                   filter(!is.na(wshdImpPCT)),'wshdImpPCT')
cdfdatawshdImpPCT_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,wshdImpPCT),by='UID') %>%
                           filter(!is.na(wshdImpPCT)),'wshdImpPCT')
cdfdatawshdImpPCT_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,wshdImpPCT),by='UID') %>%
                           filter(!is.na(wshdImpPCT)),'wshdImpPCT')
                                  
cdfdatawshdImpPCT_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,wshdImpPCT),by='UID') %>%
                           filter(!is.na(wshdImpPCT)),'wshdImpPCT')
saveRDS(cdfdatawshdImpPCT,'FinalDataAndPlots/Fish/CDF/cdfdatawshdImpPCT.RDS')
saveRDS(cdfdatawshdImpPCT_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdatawshdImpPCT.RDS')
saveRDS(cdfdatawshdImpPCT_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdatawshdImpPCT.RDS')
saveRDS(cdfdatawshdImpPCT_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdatawshdImpPCT.RDS')

cdfdataTotHab_wMD <- CDF(merge(fishTaxa,select(envData,UID,TotHab_wMD),by='UID') %>%
                   filter(!is.na(TotHab_wMD)),'TotHab_wMD')
cdfdataTotHab_wMD_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,TotHab_wMD),by='UID') %>%
                           filter(!is.na(TotHab_wMD)),'TotHab_wMD')
cdfdataTotHab_wMD_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,TotHab_wMD),by='UID') %>%
                           filter(!is.na(TotHab_wMD)),'TotHab_wMD')
                                  
cdfdataTotHab_wMD_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,TotHab_wMD),by='UID') %>%
                           filter(!is.na(TotHab_wMD)),'TotHab_wMD')
saveRDS(cdfdataTotHab_wMD,'FinalDataAndPlots/Fish/CDF/cdfdataTotHab_wMD.RDS')
saveRDS(cdfdataTotHab_wMD_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdataTotHab_wMD.RDS')
saveRDS(cdfdataTotHab_wMD_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdataTotHab_wMD.RDS')
saveRDS(cdfdataTotHab_wMD_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdataTotHab_wMD.RDS')

cdfdataSf <- CDF(merge(fishTaxa,select(envData,UID,Sf),by='UID') %>%
                   filter(!is.na(Sf)),'Sf')
cdfdataSf_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,Sf),by='UID') %>%
                           filter(!is.na(Sf)),'Sf')
cdfdataSf_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,Sf),by='UID') %>%
                           filter(!is.na(Sf)),'Sf')
                                  
cdfdataSf_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,Sf),by='UID') %>%
                           filter(!is.na(Sf)),'Sf')
saveRDS(cdfdataSf,'FinalDataAndPlots/Fish/CDF/cdfdataSf.RDS')
saveRDS(cdfdataSf_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdataSf.RDS')
saveRDS(cdfdataSf_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdataSf.RDS')
saveRDS(cdfdataSf_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdataSf.RDS')

cdfdataCl <- CDF(merge(fishTaxa,select(envData,UID,Cl),by='UID') %>%
                   filter(!is.na(Cl)),'Cl')
cdfdataCl_Mountain <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Mountain') %>%
                                  select(UID,Cl),by='UID') %>%
                           filter(!is.na(Cl)),'Cl')
cdfdataCl_Piedmont <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Piedmont') %>%
                                  select(UID,Cl),by='UID') %>%
                           filter(!is.na(Cl)),'Cl')
                                  
cdfdataCl_Coast <- CDF(merge(fishTaxa,filter(envData,BioRegion=='Coast') %>%
                               select(UID,Cl),by='UID') %>%
                           filter(!is.na(Cl)),'Cl')
saveRDS(cdfdataCl,'FinalDataAndPlots/Fish/CDF/cdfdataCl.RDS')
saveRDS(cdfdataCl_Mountain,'FinalDataAndPlots/Fish/Bioregion/Mountain/CDF/cdfdataCl.RDS')
saveRDS(cdfdataCl_Piedmont,'FinalDataAndPlots/Fish/Bioregion/Piedmont/CDF/cdfdataCl.RDS')
saveRDS(cdfdataCl_Coast,'FinalDataAndPlots/Fish/Bioregion/Coast/CDF/cdfdataCl.RDS')
...
for(i in parametersToAnalyze){
  ...
  # Mountain
  y <- tolerance_bioregion(spdata=gather(fishTaxa,GENUS,RA,-UID),
  ...
  saveRDS(y,paste('FinalDataAndPlots/Fish/Bioregion/Mountain/GAM/',as.character(names(envData)[i]),'/',
                  as.character(names(envData)[i]),'.RDS',sep=''))
  
  
   # Piedmont
  x <- tolerance_bioregion(spdata=gather(fishTaxa,GENUS,RA,-UID),
  ...
  saveRDS(x,paste('FinalDataAndPlots/Fish/Bioregion/Piedmont/GAM/',as.character(names(envData)[i]),'/',
                  as.character(names(envData)[i]),'.RDS',sep=''))
  
  # Coast
  w <- tolerance_bioregion(spdata=gather(fishTaxa,GENUS,RA,-UID),
  ...
  saveRDS(w,paste('FinalDataAndPlots/Fish/Bioregion/Coast/GAM/',as.character(names(envData)[i]),'/',
                  as.character(names(envData)[i]),'.RDS',sep=''))
}
...
```{r}
taxaList <- fishTaxa
  #readxl::read_excel('Jason/FishSpecies.xlsx',sheet='AllTaxa') %>%
  #select(StationID,Year,`Acantharchus  pomotis`:`Umbra  pygmaea`)
```


Read in environmental data. The environmental data must be a single parameter for each TITAN run.
```{r}
envData <- envData
  #readxl::read_excel('Jason/EnviroData.xlsx',sheet='stessorData') %>%
  #select(StationID, Year, DO:TotHab,MetalCCU)#, CALCIUM:MERCURY)
```

Make sure merged datasets match Jason's merge attempt. It did, won't keep rerunning
```{r}
combined <- merge(envData,taxaList,by=c('UID')) #%>% 
#  mutate(UID=paste(StationID,Year,sep="_"))
#JasonMerge <- readxl::read_excel('Jason/FishMatch.xlsx') %>% 
#  mutate(StationID= stessorData.StationID, Year = stessorData.Year)

#test <- full_join(combined,JasonMerge,by=c('StationID','Year'))
#length(unique(test$UID)) == nrow(test) # if that == the nrow of test then we are good

#saveRDS(combined,'data/FishTaxaEnv.RDS')
```


Make a taxa dataset that can be used in TITAN model. Essentially, combine StationID and Year columns to make a unique identifier that will be matched to envData. Must have min 3 taxon occurrence to run TITAN, paper suggests 5 but I went with 3 to be most inclusive of our data. Open to discussion.

```{r}
taxaListPrep <- #mutate(taxaList,UID=paste(StationID,Year,sep="_")) %>%
  filter(taxaList,UID %in% combined$UID) #%>%
  #select(-c(StationID,Year)) %>% select(UID,everything())

taxonOccurrence <- taxaListPrep %>%
  gather(taxaName,count,-UID) %>%
  mutate(taxonOccur= ifelse(count>1,1,0)) %>%
  filter(taxonOccur != 0) %>%
  group_by(taxaName)%>%
  summarise(ntaxInDB= sum(taxonOccur)) %>% 
  filter(ntaxInDB >= 5)

validTaxaList <- select(taxaListPrep,UID,taxonOccurrence$taxaName) %>%
  as.data.frame()
```

Now match envData to validTaxaList. Change rownames to UID for TITAN. That last bit cost me hours to figure out.
```{r}
validEnvData <- #mutate(envData,UID=paste(StationID,Year,sep="_")) %>%
  filter(envData,UID %in% validTaxaList$UID) %>%
  #select(-c(StationID,Year)) %>%
  as.data.frame()
row.names(validEnvData) <- validEnvData$UID
validEnvData <- select(validEnvData,-UID) 
row.names(validTaxaList) <- validTaxaList$UID
validTaxaList <- select(validTaxaList,-UID)
```


How to run all the TITAN models and save data and plots for later analysis.
```{r}
for(i in 11:length(validEnvData)){
  stressor <- names(validEnvData)[i]
  result <- titan(validEnvData[,i],validTaxaList,minSplt = 5)
  saveRDS(result,paste(stressor,'TITAN.RDS',sep="_"))
  
  titan(glades.env, glades.taxa, minSplt = 5,
  numPerm = 250, boot = TRUE, nBoot = 500, imax = FALSE,
  ivTot = FALSE, pur.cut = 0.95, rel.cut = 0.95, ncpus = 1, memory = FALSE)
  
  stressor <- names(validEnvData)[i]
  envForTitan <- data.frame(validEnvData[,i])
  names(envForTitan) <- stressor
  
  result <- titan(envForTitan,data.frame(validTaxaList),
                  minSplt = 5, numPerm = 5, boot = FALSE)#TRUE, nBoot = 5, memory = TRUE)
  # Save Taxa plot
  #for(k in 1:length()){
  #  file_name <- paste('TestDataAndPlots/Fish/Titan/',stressor,'/',tnames[k],'.jpg',sep='')
  #  jpeg(file_name)
  #  #plotTaxa(result,xlabel = prettyXlabel(parameter))
  #  dev.off()
  #}
